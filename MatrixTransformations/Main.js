...
var $author$project$Main$fShapedPolygon = {
	others: _List_fromArray(
		[
			_Utils_Tuple2(1, 0),
			_Utils_Tuple2(1, 2),
			_Utils_Tuple2(2, 2),
...
var $author$project$Matrix$newMatrix = F3(
	function (width, height, values) {
		return ((width < 1) || (height < 1)) ? $elm$core$Maybe$Nothing : (_Utils_eq(
			$elm$core$List$length(values),
			width * height) ? $elm$core$Maybe$Just(
			$author$project$Matrix$Matrix(
				{
					height: height,
					values: $elm$core$Array$fromList(values),
					width: width
				})) : $elm$core$Maybe$Nothing);
	});
...
var $author$project$Matrix$withIdentityDefault = function (maybeMatrix) {
	return A2(
...
var $author$project$Matrix$newMatrixWithDefault = F3(
	function (width, height, values) {
		return $author$project$Matrix$withIdentityDefault(
			A3($author$project$Matrix$newMatrix, width, height, values));
	});
var $author$project$Main$originalMatrix = A3(
	$author$project$Matrix$newMatrixWithDefault,
	2,
	2,
	_List_fromArray(
		[1, 0.5, 0, 1]));
...
var $author$project$Matrix$getValue = F3(
	function (x, y, matrix) {
		var m = matrix.a;
		return A2($elm$core$Array$get, x + (y * m.width), m.values);
	});
var $elm$core$Basics$append = _Utils_append;
var $author$project$Matrix$for = F3(
	function (curr, end, func) {
		return _Utils_eq(curr, end - 1) ? _List_fromArray(
			[
				func(curr)
			]) : A2(
			$elm$core$List$cons,
			func(curr),
			A3($author$project$Matrix$for, curr + 1, end, func));
	});
var $elm$core$Basics$neq = _Utils_notEqual;
var $author$project$Matrix$multiplyMatrices = F2(
	function (first, second) {
		var f = first.a;
		var s = second.a;
		if (!_Utils_eq(f.width, s.height)) {
			return $elm$core$Maybe$Nothing;
		} else {
			var resultingWidth = s.width;
			var resultingHeight = f.height;
													...
													A3(
														...
														function (n) {
															return A2(
																...
																1,
																A3(
																	$author$project$Matrix$getValue,
																	n,
																	j,
																	$author$project$Matrix$Matrix(f))) * A2(
																$elm$core$Maybe$withDefault,
																1,
																A3(
																	$author$project$Matrix$getValue,
																	i,
																	n,
																	$author$project$Matrix$Matrix(s)));
														}));
											});
									}))),
...
var $author$project$Main$transformPoint = F2(
	function (matrix, point) {
		...
		var pointMatrix = A3(
			...
			_List_fromArray(
				[x1, y1]));
		var result = $author$project$Matrix$withIdentityDefault(
			A2($author$project$Matrix$multiplyMatrices, matrix, pointMatrix));
		return _Utils_Tuple2(
			A2(
				$elm$core$Maybe$withDefault,
				0,
				A3($author$project$Matrix$getValue, 0, 0, result)),
			A2(
				$elm$core$Maybe$withDefault,
				0,
				A3($author$project$Matrix$getValue, 0, 1, result)));
	});
var $author$project$Main$transformPolygon = F2(
	function (matrix, polygon) {
		return {
			others: A2(
				$elm$core$List$map,
				$author$project$Main$transformPoint(matrix),
				polygon.others),
			start: A2($author$project$Main$transformPoint, matrix, polygon.start)
		};
	});
var $author$project$Main$init = {
	matrix: $author$project$Main$originalMatrix,
	original: $author$project$Main$fShapedPolygon,
	transformed: A2($author$project$Main$transformPolygon, $author$project$Main$originalMatrix, $author$project$Main$fShapedPolygon)
};
...
var $author$project$Matrix$setValue = F4(
	function (x, y, value, matrix) {
		...
		return $author$project$Matrix$Matrix(
			_Utils_update(
				m,
				{
					values: A3($elm$core$Array$set, x + (y * m.width), value, m.values)
				}));
	});
var $elm$core$String$toFloat = _String_toFloat;
var $author$project$Main$updateMatrixValue = F4(
	function (model, x, y, value) {
		var matrix = A4(
			$author$project$Matrix$setValue,
			x,
			y,
			A2(
				$elm$core$Maybe$withDefault,
		...
		return _Utils_update(
			...
			{
				matrix: matrix,
				transformed: A2($author$project$Main$transformPolygon, matrix, model.original)
			});
	});
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'ChangedFirst':
				var value = msg.a;
				return A4($author$project$Main$updateMatrixValue, model, 0, 0, value);
			case 'ChangedSecond':
				var value = msg.a;
				return A4($author$project$Main$updateMatrixValue, model, 1, 0, value);
			case 'ChangedThird':
				var value = msg.a;
				return A4($author$project$Main$updateMatrixValue, model, 0, 1, value);
			default:
				var value = msg.a;
				return A4($author$project$Main$updateMatrixValue, model, 1, 1, value);
		}
	});
var $author$project$Main$ChangedFirst = function (a) {
	return {$: 'ChangedFirst', a: a};
};
var $author$project$Main$ChangedFourth = function (a) {
	return {$: 'ChangedFourth', a: a};
};
var $author$project$Main$ChangedSecond = function (a) {
	return {$: 'ChangedSecond', a: a};
};
var $author$project$Main$ChangedThird = function (a) {
	return {$: 'ChangedThird', a: a};
};
...
var $author$project$Main$canvasHeight = 450;
var $author$project$Main$canvasWidth = 450;
...
var $author$project$Main$combineTwoBounds = F2(
	function (b1, b2) {
		return {
			bottom: A2($elm$core$Basics$min, b1.bottom, b2.bottom),
			left: A2($elm$core$Basics$min, b1.left, b2.left),
			right: A2($elm$core$Basics$max, b1.right, b2.right),
			top: A2($elm$core$Basics$max, b1.top, b2.top)
		};
	});
var $author$project$Main$pointToBounds = function (point) {
	var _v0 = point;
	var x = _v0.a;
	var y = _v0.b;
	return {bottom: y, left: x, right: x, top: y};
};
var $author$project$Main$getPolygonBounds = function (polygon) {
	return A3(
		$elm$core$List$foldl,
		F2(
			function (curr, acc) {
				...
				var y = _v0.b;
				return {
					bottom: A2($elm$core$Basics$min, y, acc.bottom),
					left: A2($elm$core$Basics$min, x, acc.left),
					right: A2($elm$core$Basics$max, x, acc.right),
					top: A2($elm$core$Basics$max, y, acc.top)
				};
			}),
		$author$project$Main$pointToBounds(polygon.start),
		polygon.others);
};
var $author$project$Main$getBoundsForMultiplePolygons = function (polygons) {
	if (polygons.b) {
		var first = polygons.a;
		var rest = polygons.b;
		return A3(
			$elm$core$List$foldl,
			F2(
				function (polygon, bounds) {
					return A2(
						$author$project$Main$combineTwoBounds,
						bounds,
						$author$project$Main$getPolygonBounds(polygon));
				}),
			$author$project$Main$getPolygonBounds(first),
			rest);
	} else {
		return $author$project$Main$pointToBounds(
			_Utils_Tuple2(0, 0));
	}
};
...
var $author$project$Main$cartesianToCanvas = F4(
	function (centerX, centerY, scale, point) {
		var _v0 = point;
		var x = _v0.a;
		var y = _v0.b;
		return _Utils_Tuple2(centerX + (scale * x), centerY - (scale * y));
	});
...
var $author$project$Main$renderPolygon = F4(
	function (polygon, centerX, centerY, scale) {
		var convertor = A3($author$project$Main$cartesianToCanvas, centerX, centerY, scale);
		return A2(
			$joakin$elm_canvas$Canvas$path,
			convertor(polygon.start),
			A2(
				$elm$core$List$map,
				$joakin$elm_canvas$Canvas$lineTo,
				A2($elm$core$List$map, convertor, polygon.others)));
	});
...
var $author$project$Main$for = F3(
	function (current, end, _function) {
		return _Utils_eq(current, end - 1) ? _List_fromArray(
			[
				_function(current)
			]) : A2(
			$elm$core$List$cons,
			_function(current),
			A3($author$project$Main$for, current + 1, end, _function));
	});
...
var $author$project$Main$renderXLabels = F5(
	function (interval, centerX, centerY, scale, labelTickLength) {
		var numberOfTicks = $elm$core$Basics$floor(($author$project$Main$canvasWidth / scale) / interval);
		var fontSize = 12;
		var firstNumber = (-interval) * $elm$core$Basics$floor((centerX / scale) / interval);
		var margin = centerX + (scale * firstNumber);
		return A2(
			$joakin$elm_canvas$Canvas$group,
			_List_Nil,
			A3(
				$author$project$Main$for,
				0,
				numberOfTicks,
				function (i) {
					var x = margin + ((i * interval) * scale);
					var label = $elm$core$Debug$toString(firstNumber + (i * interval));
					return A2(
						$joakin$elm_canvas$Canvas$group,
						_List_fromArray(
							[
							...
							[
								...
								_List_fromArray(
									[
										...
										_Utils_Tuple2(x, centerY - (labelTickLength / 2)),
										1,
										labelTickLength)
									])),
								A3(
								...
								_List_fromArray(
									[
										...
										{family: 'Segoe UI', size: fontSize}),
										$joakin$elm_canvas$Canvas$Settings$Text$align($joakin$elm_canvas$Canvas$Settings$Text$Center)
									]),
								_Utils_Tuple2(x, ((centerY + labelTickLength) + 1) + (fontSize / 2)),
								(label === '0') ? '' : label)
							]));
				}));
	});
var $joakin$elm_canvas$Canvas$Settings$Text$Left = {$: 'Left'};
var $author$project$Main$renderYLabels = F5(
	function (interval, centerX, centerY, scale, labelTickLength) {
		var numberOfTicks = $elm$core$Basics$floor(($author$project$Main$canvasHeight / scale) / interval);
		var fontSize = 12;
		var firstNumber = interval * $elm$core$Basics$floor((centerY / scale) / interval);
		var margin = centerY - (scale * firstNumber);
		return A2(
			$joakin$elm_canvas$Canvas$group,
			_List_Nil,
			A3(
							...
							[
								...
								_List_fromArray(
									[
										A3(
										$joakin$elm_canvas$Canvas$rect,
										_Utils_Tuple2(centerX - (labelTickLength / 2), y),
										labelTickLength,
										1)
									])),
								A3(
								...
								_Utils_Tuple2((centerX + labelTickLength) + 1, y + (fontSize / 2)),
								(label === '0') ? '' : label)
							]));
				}));
	});
...
var $author$project$Main$renderCanvas = function (model) {
	var labelTickLength = 10;
	var bounds = $author$project$Main$getBoundsForMultiplePolygons(
		_List_fromArray(
			[model.original, model.transformed]));
	var xSpan = bounds.right - bounds.left;
	var ySpan = bounds.top - bounds.bottom;
	var maxSpan = A2($elm$core$Basics$max, ySpan, xSpan);
	var interval = A2(
		$elm$core$Basics$pow,
		2.0,
		$elm$core$Basics$floor(
			A2($elm$core$Basics$logBase, 2, maxSpan)) - 2);
	var scale = $author$project$Main$canvasWidth / (maxSpan * 1.25);
	var xMargin = ($author$project$Main$canvasHeight - (scale * xSpan)) / 2;
	var centerX = xMargin + (scale * $elm$core$Basics$abs(bounds.left));
	var yMargin = ($author$project$Main$canvasHeight - (scale * ySpan)) / 2;
	var centerY = yMargin + (scale * bounds.top);
	return A3(
		...
		_List_fromArray(
			[
				...
				A5($author$project$Main$renderXLabels, interval, centerX, centerY, scale, labelTickLength),
				A5($author$project$Main$renderYLabels, interval, centerX, centerY, scale, labelTickLength),
				A2(
				$joakin$elm_canvas$Canvas$shapes,
				_List_Nil,
				_List_fromArray(
					[
						A3(
						$joakin$elm_canvas$Canvas$rect,
						_Utils_Tuple2(0, centerY),
						$author$project$Main$canvasWidth,
						1),
						A3(
						$joakin$elm_canvas$Canvas$rect,
						_Utils_Tuple2(centerX, 0),
						1,
						$author$project$Main$canvasHeight)
					])),
				A2(
				...
				_List_fromArray(
					[
						...
						A4($avh4$elm_color$Color$rgba, 0.5, 1, 0, 0.3)),
						$joakin$elm_canvas$Canvas$Settings$stroke(
						A4($avh4$elm_color$Color$rgba, 0.5, 1, 0, 0.6))
					]),
				_List_fromArray(
					[
						A4($author$project$Main$renderPolygon, model.original, centerX, centerY, scale)
					])),
				A2(
				$joakin$elm_canvas$Canvas$shapes,
				_List_fromArray(
					[
						$joakin$elm_canvas$Canvas$Settings$fill(
						A4($avh4$elm_color$Color$rgba, 0.5, 0, 1, 0.3)),
						$joakin$elm_canvas$Canvas$Settings$stroke(
						A4($avh4$elm_color$Color$rgba, 0.5, 0, 1, 0.6))
					]),
				_List_fromArray(
					[
						A4($author$project$Main$renderPolygon, model.transformed, centerX, centerY, scale)
					]))
			]));
};
...
var $author$project$Main$renderNumberInput = F4(
	function (msg, x, y, model) {
		return A2(
			$elm$html$Html$td,
			_List_Nil,
			_List_fromArray(
						...
						[
							...
							$elm$core$Debug$toString(
								A2(
									$elm$core$Maybe$withDefault,
									0,
									A3($author$project$Matrix$getValue, x, y, model.matrix))))
						]),
					_List_Nil)
				]));
	});
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$Attributes$id('matrix-label')
									]),
								_List_fromArray(
									[
										$elm$html$Html$text('Transformation matrix:')
									])),
								A2(
								$elm$html$Html$td,
								_List_Nil,
									...
									[
										...
										_List_fromArray(
											[
												...
												_List_fromArray(
													[
														A4($author$project$Main$renderNumberInput, $author$project$Main$ChangedFirst, 0, 0, model),
														A4($author$project$Main$renderNumberInput, $author$project$Main$ChangedSecond, 1, 0, model)
													])),
												A2(
												$elm$html$Html$tr,
												_List_Nil,
												_List_fromArray(
													[
														A4($author$project$Main$renderNumberInput, $author$project$Main$ChangedThird, 0, 1, model),
														A4($author$project$Main$renderNumberInput, $author$project$Main$ChangedFourth, 1, 1, model)
													]))
											]))
									]))
							]))
